...
from script.change import change_name
from script.get import get_name
...
class SnifferDHCP(object):
	...
	def run(self,arg):
		...
		while True:
			sniff(filter=self.filter,prn=self.detect_dhcp,count=4)
	...
	def detect_dhcp(self,pkt):
		...
		if DHCP in pkt:
			...
			if pkt[DHCP].fields['options'][0][1] == 5 or pkt[DHCP].fields['options'][0][1] == 6:
				...
				br = getbr(avail_ip)
				# print (host_info_tmp)
				host_info_tmp[client_mac] =  {}
				host_info_tmp[client_mac]['client_ip'] = client_ip
				host_info_tmp[client_mac]['avail_ip'] =  avail_ip
				host_info_tmp[client_mac]['server_ip'] =  server_ip
				host_info_tmp[client_mac]['gia_ip'] =  gia_ip
				
				# try:
				# 	if hostname_vendor[client_mac]['hostname'] is not None:
				# 		host_info_tmp[client_mac]['hostname'] = hostname_vendor[client_mac]['hostname']
				# 		del hostname_vendor[client_mac]['hostname']
				# except Exception:
				# 	pass
				# try:	
				# 	if hostname_vendor[client_mac]['vendor'] is not None:
				# 		host_info_tmp[client_mac]['vendor'] = hostname_vendor[client_mac]['vendor']
				# 		del hostname_vendor[client_mac]
				# except Exception:
				# 	pass
				# change_name(host_info_tmp)
		...
		# print (id(host_info),host_info,'main')
		# sniff(filter=self.filter,prn=self.detect_dhcp,callback=True)
		# name = get_name()
		# print(name)

def start_gatway(pool):
	
	sniffThread = SnifferDHCP()
	pool.put(sniffThread.run,len(pool.generate_list))
